...
class AddonUpdaterCheckNow(bpy.types.Operator):
    bl_label = "Check for an update"
    bl_idname = updater.addon + ".updater_check_now"
    bl_description = "Check for an update to SculptMate"
    bl_options = {'REGISTER', 'INTERNAL'}
...
def register(bl_info):
    ...
    updater.user = "shravan-d"

    # Choose your own repository, must match git name for GitHUb and Bitbucket,
    # for GitLab use project ID (numbers only).
    updater.repo = "SculptMate"

    # updater.addon = # define at top of module, MUST be done first

    # Website for manual addon download, optional but recommended to set.
    updater.website = "https://github.com/shravan-d/SculptMate/releases"
    ...
    updater.backup_ignore_patterns = ["__pycache__", "*.pth", "*.pt", "*.gif",]
    ...
    updater.overwrite_patterns = ["*.png", "*.jpg", "README.md", "LICENSE.txt", "requirements.txt"]
